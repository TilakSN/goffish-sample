...
public class KCoreSetIteration extends AbstractSubgraphComputation<LongWritable, LongWritable, LongWritable, KCoreSetIterationMessage, LongWritable, LongWritable, LongWritable> implements ISubgraphWrapup {
    ...
    @Override
    public void compute(Iterable<IMessage<LongWritable, KCoreSetIterationMessage>> iMessages) throws IOException {
        if (getSuperstep() == 0) {
            ...
            for (IMessage<LongWritable, KCoreSetIterationMessage> iMessage : iMessages)
    ...
    private void informNeighbors(long vertexId, int core) {
        for (long sgId : subGraphIds.get(vertexId))
            sendMessage(new LongWritable(sgId), new KCoreSetIterationMessage(vertexId, core));
    }

    private void handleMessage(KCoreSetIterationMessage message) {
...
class KCoreSetIterationMessage implements Writable {
    private long vertexId;
    private int core;

    public KCoreSetIterationMessage() {
    }

    public KCoreSetIterationMessage(long vertexId, int core) {
