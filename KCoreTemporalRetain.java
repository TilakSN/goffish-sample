...
public class KCoreTemporalRetain extends AbstractSubgraphComputation<LongWritable, LongWritable, LongWritable, KCoreTemporalRetainMessage, LongWritable, LongWritable, LongWritable> implements ISubgraphWrapup {
    ...
    @Override
    public void compute(Iterable<IMessage<LongWritable, KCoreTemporalRetainMessage>> iMessages) throws IOException {
        if (getSuperstep() == 0) {
            ...
            coreMap = null;
            for (IMessage<LongWritable, KCoreTemporalRetainMessage> iMessage : iMessages)
    ...
    private void informNeighbors(long vertexId, int core) {
        for (long sgId : subGraphIds.get(vertexId))
            sendMessage(new LongWritable(sgId), new KCoreTemporalRetainMessage(vertexId, core));
    }

    private void handleMessage(KCoreTemporalRetainMessage message) {
...
class KCoreTemporalRetainMessage implements Writable {
    private long vertexId;
    private int core;

    public KCoreTemporalRetainMessage() {
    }

    public KCoreTemporalRetainMessage(long vertexId, int core) {
