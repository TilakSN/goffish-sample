...
public class KCoreTemporal extends AbstractSubgraphComputation<LongWritable, LongWritable, LongWritable, KCoreTemporalMessage, LongWritable, LongWritable, LongWritable> implements ISubgraphWrapup {

    private static final Object lock = new Object();
    private static Map<Long, Integer> coreMap = null;
    private static final String path = "/user/humus/FBTempIn2/";
    ...
    @Override
    public void compute(Iterable<IMessage<LongWritable, KCoreTemporalMessage>> iMessages) throws IOException {
        if (getSuperstep() == 0) {
            ...
            coreMap = null;
            for (IMessage<LongWritable, KCoreTemporalMessage> iMessage : iMessages)
    ...
    private void converge_locally() {
        Set<LongWritable> verticesToProcess = new HashSet<>();
        for (IVertex<LongWritable, LongWritable, LongWritable, LongWritable> vertex : getSubgraph().getLocalVertices()) {
            verticesToProcess.remove(vertex.getVertexId());
            if (processVertex(vertex, false, false))
                for (IEdge<LongWritable, LongWritable, LongWritable> edge: vertex.getOutEdges())
                    verticesToProcess.add(edge.getSinkVertexId());
        }
        while (!verticesToProcess.isEmpty()) {
            Set<LongWritable> pendingVertices = new HashSet<>();
            for (LongWritable vertexId: verticesToProcess) {
                ...
                if (!vertex.isRemote() && processVertex(vertex, false, false))
                    for (IEdge<LongWritable, LongWritable, LongWritable> edge: vertex.getOutEdges())
                        pendingVertices.add(edge.getSinkVertexId());
            }
    ...
    private boolean processVertex(IVertex<LongWritable, LongWritable, LongWritable, LongWritable> vertex) {
        return processVertex(vertex, true, true);
    }

    private boolean processVertex(IVertex<LongWritable, LongWritable, LongWritable, LongWritable> vertex, boolean useCore, boolean inform) {
        long vertexId = vertex.getVertexId().get();
        int core = cores.get(vertexId);
        int degree = degrees.get(vertexId);
        int max = useCore ? core : degree;
        int[] count = new int[max + 1];
        for (IEdge<LongWritable, LongWritable, LongWritable> edge : vertex.getOutEdges())
            count[Math.min(max, cores.get(edge.getSinkVertexId().get()))]++;
    ...
    private void informNeighbors(long vertexId, int core) {
        for (long sgId : subGraphIds.get(vertexId))
            sendMessage(new LongWritable(sgId), new KCoreTemporalMessage(vertexId, core));
    }

    private void handleMessage(KCoreTemporalMessage message) {
...
class KCoreTemporalMessage implements Writable {
    private long vertexId;
    private int core;

    public KCoreTemporalMessage() {
    }

    public KCoreTemporalMessage(long vertexId, int core) {
