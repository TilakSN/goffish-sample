...
public class KCoreTemporalUpdate extends AbstractSubgraphComputation<LongWritable, LongWritable, LongWritable, KCoreTemporalUpdateMessage, LongWritable, LongWritable, LongWritable> implements ISubgraphWrapup {

    private static final Object lock = new Object();
    private static Map<Long, Integer> coreMap = null;
    private static final String path = "/user/humus/FBTempIn3/";
    ...
    @Override
    public void compute(Iterable<IMessage<LongWritable, KCoreTemporalUpdateMessage>> iMessages) throws IOException {
        if (getSuperstep() == 0) {
            ...
            coreMap = null;
            for (IMessage<LongWritable, KCoreTemporalUpdateMessage> iMessage : iMessages)
    ...
    private void informNeighbors(long vertexId, int core) {
        for (long sgId : subGraphIds.get(vertexId))
            sendMessage(new LongWritable(sgId), new KCoreTemporalUpdateMessage(vertexId, core));
    }

    private void handleMessage(KCoreTemporalUpdateMessage message) {
...
class KCoreTemporalUpdateMessage implements Writable {
    private long vertexId;
    private int core;

    public KCoreTemporalUpdateMessage() {
    }

    public KCoreTemporalUpdateMessage(long vertexId, int core) {
